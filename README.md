...
个人中心入口位于左下角头像，提供恢复码、加密云备份同步、新设备扫码恢复、设备撤销、本地解锁口令修改、锁定退出和清除本机数据。扫码内容仅存在 URL Fragment 中，不会随 HTTP 请求发送给服务器；新设备读取后会立即清除地址栏中的敏感片段。二维码等同于恢复密钥，请勿截图、转发或让他人扫描。单条消息举报位于消息右键或长按菜单；确认后才会把该条消息明文提交给管理员。
