...
- 用户名注册、设备身份、恢复码、本地加密保险库与新设备扫码恢复
...
尚未完成生产级验收：完整多设备互操作、独立密码学审计、后台权限分级和大规模压力测试。因此项目持续显示 `experimental` 标识。
...
`--port 8088` 就是反向代理上游端口。需要其他端口时直接替换数字，例如使用 `9000`：

```bash
./deploy.sh --port 9000
```

不写 `--port` 时，部署脚本会在终端询问，直接回车使用默认 `8088`。部署成功后，反向代理上游分别是 `http://127.0.0.1:8088` 或 `http://127.0.0.1:9000`。
...
当前版本没有内置的“绑定数据库”图形化页面；已提供第一版运营管理后台。默认一键部署已经自动创建并绑定 PostgreSQL，连接关系由 [`compose.deploy.yml`](compose.deploy.yml) 和服务器上的私密 `.env` 管理：
...
部署脚本会生成随机后台路径和独立管理员令牌，不固定使用 `/admin`：

```dotenv
COVECHAT_ADMIN_PATH=/manage-随机字符串
COVECHAT_ADMIN_TOKEN=<自动生成的高强度令牌>
```

部署完成后终端会显示完整后台地址。也可以在服务器查询（不要把结果发送给其他人）：

```bash
cd /opt/covechat
grep -E '^COVECHAT_ADMIN_(PATH|TOKEN)=' .env
```

后台提供运行概览、账户停用和恢复、设备撤销、举报处理及审计日志。自定义路径只是减少自动扫描，真正的访问控制来自管理员令牌；令牌只保存在当前浏览器会话中。

修改后台路径或轮换令牌后执行：

```bash
docker compose --env-file .env -f compose.deploy.yml up -d --build web api
```

个人中心入口位于左下角头像，提供恢复码、加密云备份同步、新设备扫码配对、设备撤销、本地解锁口令修改、锁定退出和清除本机数据。安全配对由新设备生成十分钟有效的一次性二维码，二维码不包含恢复密钥；已登录设备确认后，双方通过临时 P-256 ECDH 密钥派生 AES-GCM 密钥，服务器只中转临时密文，恢复完成后立即销毁配对记录。单条消息举报位于消息右键或长按菜单；确认后才会把该条消息明文提交给管理员。
